...
test("manual recovery is limited to valid lifecycle states", () => {
  assert.match(adminOrderApi, /EMAIL_KINDS = new Set\(\["confirmation", "shipping", "refund"\]\)/);
...
test("admin recovery covers confirmation invoice shipping refund and credit note", () => {
  for (const label of ["Confirmation", "Facture", "Expédition", "Remboursement", "Avoir"]) {
