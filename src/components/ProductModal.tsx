...
function stockCopy(stock: number, language: Language) {
...
export function ProductModal({
  ...
  onPackagingSelect: (key: PackagingKey) => void;
  ...
  const image = gallery[Math.min(imageIndex, gallery.length - 1)] || gallery[0] || "/product-placeholder.svg";
  ...
  }, [gallery.length, onClose, opener, setImageIndex]);
              ...
              <div className="product-cart-confirmation">
                ...
                <div className="product-cart-actions">
                  <div className="product-inline-qty" aria-label={language === "fr" ? "Quantité dans le panier" : "Quantity in cart"}>
                    <button type="button" onClick={onDecreaseCartQuantity} aria-label={language === "fr" ? "Diminuer la quantité" : "Decrease quantity"}>−</button>
