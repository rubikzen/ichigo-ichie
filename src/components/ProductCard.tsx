...
import type { ProductModalPackageOption, ProductModalPackagingKey } from "./ProductDetailModal";
...
type PackagingKey = ProductModalPackagingKey;
...
function ProductCardStateful({ product }: { product: Product }) {
  ...
  const packageOptions = useMemo<ProductModalPackageOption[]>(() => {
    const seen = new Set<PackagingKey>();
    return selectableVariants.reduce<ProductModalPackageOption[]>((rows, item) => {
  ...
  const canAdd = hasStock && !stockLimitReached && product.option_groups.every((group) => {
    ...
    const minimum = group.required ? Math.max(1, group.min_select) : Math.max(0, group.min_select);
    return selectedCount >= minimum && selectedCount <= group.max_select;
  });
