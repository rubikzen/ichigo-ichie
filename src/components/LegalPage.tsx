...
function renderLegalContent(value: string): ReactNode[] {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("### ")) {
      blocks.push(<h3 key={key++}>{line.slice(4)}</h3>); i += 1; continue;
    }
    if (line.startsWith("## ") || line.startsWith("# ")) {
      blocks.push(<h2 key={key++}>{line.replace(/^#{1,2}\s+/, "")}</h2>); i += 1; continue;
    }
    if (line.startsWith("- ")) {
      const items: string[] = [];
      while (i < lines.length && lines[i].trim().startsWith("- ")) {
        items.push(lines[i].trim().slice(2)); i += 1;
      }
...
export function LegalPage({ titleKey, bodyKey }: { titleKey: string; bodyKey: string }) {
  ...
  const pick = (base: string) => settings[`${base}_${language}`] || settings[`${base}_fr`] || "";
  const body = pick(bodyKey);
