...
export function AdminCatalog({
  ...
  function toggleQuickVisibility(product: AdminProduct) {
    ...
    if (!product.active && kind === "shop" && productContentIssues(product).length > 0) {
  ...
  function renderQuickProductRow(product: AdminProduct, index: number, orderedProducts: AdminProduct[]) {
    ...
    return <article className={`quick-product-row zone-${catalogZone} ${!product.active ? "is-hidden" : ""}`} key={product.id}>
      ...
      <button type="button" className={`quick-visibility ${product.active ? "active" : ""} ${!product.active && catalogZone === "shop" && contentIssues.length > 0 ? "blocked-by-content-v456" : ""}`} title={!product.active && catalogZone === "shop" && contentIssues.length > 0 ? "Corrigez le contenu avant de publier" : product.active ? "Masquer le produit" : "Publier le produit"} onClick={() => toggleQuickVisibility(product)}>{product.active ? "Visible" : "Masqué"}</button>
      ...
      {advancedOpen && selectedId && <div className="quick-detail-panel">
        ...
        <div className="quick-detail-drawer" role="dialog" aria-modal="true" aria-label="Détails du produit">
          <form onSubmit={saveProduct}>
            ...
            {message && <p className={message.includes("✓") ? "save-message success" : "save-message"}>{message}</p>}
            {draftCategory?.kind === "shop" && !productDraft.active && draftContentIssues.length > 0 && <div className="publish-guard-banner-v456" role="status"><strong>Publication verrouillée</strong><span>Corrigez les {draftContentIssues.length} point{draftContentIssues.length > 1 ? "s" : ""} signalé{draftContentIssues.length > 1 ? "s" : ""} avant de rendre ce produit visible.</span></div>}
